...
namespace esgstl {
			...
			while (x != root) {
				...
				if (x == x_parent->left) {
					...
					// 兄黑
						...
						// 双侄黑
						if ((x_shunzhi == nullptr || !rb_tree_is_red(x_shunzhi)) 
							&& (x_duizhi == nullptr || !rb_tree_is_red(x_duizhi))) {
							// 直接兄变红，父变黑
						...
						// 顺侄黑
						else if (x_shunzhi == nullptr || !rb_tree_is_red(x_shunzhi)) {
							// 旋转父节点，兄变成父的颜色，父侄变黑
							rb_tree_left_rotate(x_parent,root);
							x_brother->color = x_parent->color;
							rb_tree_set_black(x_parent);
							rb_tree_set_black(x_duizhi);
							break;// 递归出口2:顺侄黑，旋父，兄变父色，父侄变黑
						}
						// 对侄黑
						else {
							// 旋转兄节点，然后互换兄与顺侄节点的颜色，将其转化为对侄
	...
	public:
		...
		//				std::cout << node->get_node_ptr()->value.second << '\t';
		//			}
		//			std::cout << std::endl;
		//		}
		//	}
	...
	insert_multi(const value_type& value) {
		...
		return insert_value_at(p.first, value, p.second);
	}

	template<class T, class Compare>
	inline esgstl::pair<typename typename rb_tree<T, Compare>::iterator, bool>
	rb_tree<T, Compare>::
	insert_unique(const value_type& value) {
		ESGSTL_LENGTH_ERROR_IF(node_count_ > max_size() - 1, "rb_tree<T, Comp>'s size too big");
		auto p = get_insert_unique_pos(value_traits::get_key(value));
		if (p.second) return esgstl::make_pair(insert_value_at(p.first.first, value, p.first.second), true);
	...
	template<class T, class Compare>
	inline typename rb_tree<T, Compare>::size_type
	rb_tree<T, Compare>::
	...
	erase(iterator begin, iterator end) {
		if (begin == this->begin() && end == this->end()) clear();
		else {
			while (begin != end) {
				erase(begin++);
			}
	...
	template<class T, class Compare>
	inline typename rb_tree<T, Compare>::const_iterator
	rb_tree<T, Compare>::
	...
	template<class T, class Compare>
	inline typename rb_tree<T, Compare>::base_node_ptr
	rb_tree<T, Compare>::
