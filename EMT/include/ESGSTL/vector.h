...
#undef min
	...
	public:
		...
		void assign(std::initializer_list<value_type> ilist) { copy_assign(ilist.begin(), ilist.end(), esgstl::forward_iterator_tag{}); }
	...
	vector<T>& vector<T>::operator=(const vector& rhs) {
		if (this != &rhs) {
			copy_assign(rhs.begin_, rhs.end_, esgstl::forward_iterator_tag{});
		}
		return *this;
	}
	...
	void vector<T>::
		...
		const size_type len = esgstl::distance(first, last);
		if (len > capacity()) {
			vector tmp(first, last);
			swap(tmp);
		}
		else if (size() >= len) {
			auto new_end = esgstl::copy(first, last, begin_);
			data_allocator::destory(new_end, end_);
			end_ = new_end;
		}
		else {
			auto mid = first;
			esgstl::advance(mid, size());
			esgstl::copy(first, mid, begin_);
			auto new_end = esgstl::uninitialized_copy(mid, last, end_);
			end_ = new_end;
		}
	}
	...
	inline void vector<T>::reallocate_emplace(iterator pos, Args && ...args) {
		...
		try
		{
			new_end = esgstl::uninitialized_move(begin_, pos, new_begin);
			data_allocator::construct(esgstl::address_of(*new_end), esgstl::forward<Args>(args)...);
			++new_end;
			new_end = esgstl::uninitialized_move(pos, end_, new_end);
		}
		catch (...)
		{
	...
	inline void vector<T>::copy_insert(iterator pos, Iter first, Iter last) {
		...
		const auto n = static_cast<size_type>(esgstl::distance(first, last));
		if (size() > capacity() - n) {
			...
			try {
				...
				// 失败的话析构并回收所分配的内存
				destory_and_recover(newbeg, newend, newcap);
				throw;
			}
			...
			if (after_elems > n) {
				...
				end_ = esgstl::uninitialized_copy(end_ - n, end_, end_);
				esgstl::move_backward(pos, old_end - n, old_end);
				esgstl::uninitialized_copy_n(first, n, pos);
			}
			else {
	...
	inline void vector<T>::reallocate_insert(iterator pos, const value_type& value) {
		...
		try {
			newend = esgstl::uninitialized_move(begin_, pos, newbeg);
			data_allocator::construct(newend, value_copy);
			++newend;
			newend = esgstl::uninitialized_move(pos, end_, newend);
		}
		catch (const std::exception&) {
	...
	inline typename vector<T>::iterator
		...
		{ // 如果备用空间大于等于增加的空间
			...
			if (after_elems > n)
			{
				...
				end_ += n;
				esgstl::move_backward(pos, old_end - n, old_end);
				esgstl::uninitialized_fill_n(pos, n, value_copy);
			}
			else
			{
				end_ = esgstl::uninitialized_fill_n(end_, n - after_elems, value_copy);
				end_ = esgstl::uninitialized_move(pos, old_end, end_);
				esgstl::uninitialized_fill_n(pos, after_elems, value_copy);
			}
		}
		else
		{ // 如果备用空间不足
			...
			{
				new_end = esgstl::uninitialized_move(begin_, pos, new_begin);
				new_end = esgstl::uninitialized_fill_n(new_end, n, value);
				new_end = esgstl::uninitialized_move(pos, end_, new_end);
			}
			catch (...)
			{
