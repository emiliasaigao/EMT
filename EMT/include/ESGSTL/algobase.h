...
namespace esgstl {
	...
	typename std::enable_if<
		!std::is_pointer<OutputIter>::value || 
		!std::is_integral<typename std::remove_pointer<OutputIter>::type>::value, OutputIter>::type
	...
	void unchecked_fill_cat(RandomIter first, RandomIter last, const T& value,
		...
		esgstl::fill_n(first, last - first, value);
	}
	...
	// 针对C风格字符串的特化版本
	inline bool lexicographical_compare(unsigned char* first1, unsigned char* last1,
