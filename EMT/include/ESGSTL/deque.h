...
#define DEQUE_MAP_INIT_SIZE 8
	...
	template<class T>
	inline deque<T>& deque<T>::operator=(deque&& rhs) noexcept {
		deque tmp(rhs);
		swap(tmp);
		return *this;
	}
	...
	template<class T>
	inline typename deque<T>::iterator deque<T>::erase(const_iterator pos) {
	...
	template<class T>
	inline typename deque<T>::map_pointer deque<T>::create_map(size_type map_size) {
	...
	deque<T>::fill_insert(const_iterator pos, size_type n, const value_type& value) {
		...
		if (elems_before < size() / 2) {
			...
			p1 = begin_ + elems_before;
			auto e2 = e1 + n;
			size_type elems_after = end_ - pos;
			try {
				end_ = e2;
				if (n > elems_after) {
	...
	template<class T>
	template<class ...Args>
	inline typename deque<T>::iterator deque<T>::emplace(const_iterator pos, Args && ...args) {
		if (pos.cur == begin_.cur) {
			emplace_front(esgstl::forward<Args>(args)...);
			return begin_;
		}
		else if (pos.cur == end_.cur) {
			emplace_back(esgstl::forward<Args>(args)...);
			return end_;
		}
		else {
