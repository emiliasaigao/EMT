...
namespace esgstl {
	...
	public:
		...
		map(map&& rhs) noexcept : tree_(esgstl::move(rhs.tree_)) {}
	...
	public:
		...
		mapped_type& operator[](const key_type& key) {
			...
			if (it == end() || key_comp()(it->first, key)) {
				emplace_hint(it, key, mapped_type());
			}
			return it->second;
		}
		...
		mapped_type& operator[](key_type&& key) {
			iterator it = lower_bound(key);
			if (it == end() || key_comp()(it->first, key)) {
				emplace_hint(it, esgstl::move(key), mapped_type());
			}
			return it->second;
		}
	...
	public:
		...
		multimap(multimap&& rhs) noexcept : tree_(esgstl::move(rhs.tree_)) {}
