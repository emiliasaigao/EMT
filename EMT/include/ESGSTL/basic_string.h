...
#define STRING_INIT_SIZE 32
	...
	void basic_string<CharType, CharTraits>::
		...
		auto n = esgstl::distance(first, last);
		const auto init_size = esgstl::max(static_cast<size_type>(STRING_INIT_SIZE), static_cast<size_type>(n + 1));
		try
		{
			buffer_ = data_allocator::allocate(init_size);
			size_ = n;
	...
	basic_string<CharType, CharTraits>& 
		...
		if ((static_cast<difference_type>(capacity_ - size_) < static_cast<difference_type>(n))) {
			reallocate(n);
		}
	...
	typename basic_string<CharType, CharTraits>::size_type 
		...
		for (auto i = buffer_ + pos; i != endIndex; ++i) {
			...
			if (size > static_cast<size_type>(endIndex - i) || i == buffer_ + size_) return npos;
			auto j = s;
			for (; j != s + size; ++j) {
				if (*j != *(i+(j-s))) break;
			}
	...
	typename basic_string<CharType, CharTraits>::size_type 
		...
		const size_type size = str.size_;
		auto endIndex = end();
		if (size == 0) return 0;
		for (auto i = buffer_ + pos; i != endIndex; ++i) {
			if (size > static_cast<size_type>(endIndex - i)) return npos;
			auto j = str.buffer_;
			for (; j != str.end(); ++j) {
	...
	typename basic_string<CharType, CharTraits>::size_type 
		...
		return find_last_of(s, pos, size_);
	}
	...
	basic_string<CharType, CharTraits>
		operator+(basic_string<CharType, CharTraits>&& lhs,
			const basic_string<CharType, CharTraits>& rhs) {
		basic_string<CharType, CharTraits> tmp(esgstl::move(lhs));
		tmp.append(rhs);
		return tmp;
	}
	...
	basic_string<CharType, CharTraits>
		...
		basic_string<CharType, CharTraits> tmp(esgstl::move(rhs));
